        ...
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            @foreach($list as $v)
                <tr class="text-c">
                    ...
                    <td>{{$v->recruitment_id}}</td>
                    <td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">{{$v->recruitment_name}}</u></td>
                    <td>{{$v->recruitment_sex}}</td>
                    <td>{{$v->recruitment_age}}</td>
                    <td >{{$v->recruitment_tel}}</td>

                    <td class="text-l" style="text-align: center">{{$v->recruitment_scope}}</td>
                    <td>{{date( "Y-m-d H:i:s",$v->recruitment_time )}}</td>
                    @if($v->recruitment_tag==10)
                        <td class="td-status">
                    ...
                    <td class="td-manage">
                        @if($v->recruitment_tag==10)
                            <a style="text-decoration:none" onClick="member_stop(this,'{{$v->recruitment_id}}')" href="javascript:;" title="已查看"><i class="Hui-iconfont">&#xe631;</i></a>
                        @else
                            <a style="text-decoration:none" onClick="member_start(this,'{{$v->recruitment_id}}')" href="javascript:;" title="待查看"><i class="Hui-iconfont">&#xe6e1;</i></a>
                        @endif
                        <a title="删除" href="javascript:;" onclick="member_del(this,'{{$v->recruitment_id}}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    </td>
                </tr>
            @endforeach
...
<script type="text/javascript">
    ...
    function member_stop(obj,id){
        layer.confirm('确认修改为待查看？',function(index){
